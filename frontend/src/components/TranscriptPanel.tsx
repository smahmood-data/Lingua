...
import { downloadText, formatTranscript, speakerLabel } from '../lib/sessionHistory'
import { languageMetaFromCode } from '../types'
import { SummaryPanel } from './SummaryPanel'
import './TranscriptPanel.css'

export function TranscriptPanel({ session, onClose, onSaveSession }: { session: SavedSession; onClose: () => void; onSaveSession: (session: SavedSession) => void }) {
  ...
  if (summarizing) return <SummaryPanel session={session} onClose={() => setSummarizing(false)} onSaveSession={onSaveSession} />
  return <aside className="transcript-panel" aria-label="Saved transcript">
    ...
    <div className="transcript-copy">
      {session.turns.map((turn) => <article key={turn.id} className="transcript-turn">
        <p className="transcript-speaker">{speakerLabel(turn, session.targetLanguage)}</p>
