...
| 2026-08-30 | Adil / team review pending | OpenAI Codex | Issue #31 Vercel deployment repair | Audited the live aliases, GitHub homepage, deployment records, environment failure, and post-#39 preview; added deterministic Vercel build settings, a token-redacting smoke check, and an owner runbook for the remaining account configuration. | The repository-side configuration and checks are verified locally; a Vercel owner must apply the documented root, secret, firewall, domain, and redeploy steps before completing the live microphone smoke test. |
